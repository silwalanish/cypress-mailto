...
export const Route = createFileRoute("/formAction")({
  component: RouteComponent,
});

function RouteComponent() {
  const fromRef = React.useRef<HTMLFormElement>(null);
  const hiddenFormRef = React.useRef<HTMLFormElement>(null);
  const [action, setAction] = useState("");

  useEffect(() => {
    if (action && hiddenFormRef.current) {
      hiddenFormRef.current?.submit();
    }
  }, [hiddenFormRef, action]);
  ...
  return (
    <>
      ...
      <form style={{ display: 'none' }} action={action} ref={hiddenFormRef} method="GET" target="sendMail" data-testid="email-form"></form>
      <form
        ref={fromRef}
      >
        <div>
